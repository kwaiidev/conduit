...
function distanceToTargetBounds(target: SnapTarget, x: number, y: number): number {
...
function closestPointOnTargetBounds(
  target: SnapTarget,
  x: number,
  y: number,
  inset = 0
  ...
  React.useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      if (liveLockedTarget) {
        ...
        if (
          (distToLock > RELEASE_RADIUS_PX && speed > SLOW_SPEED_PX_PER_MS * 1.75)
