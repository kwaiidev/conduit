...
function scoreSystemPriority(kind) {
...
function buildClusters(targets) {
    const clusters = [];
...
function cycleInCluster(targets, lockedId) {
    if (!lockedId) {
        return null;
    }
...
export default function SnapCursorLayer() {
    ...
    const domTargetsRef = React.useRef([]);
    const systemTargetsRef = React.useRef([]);
    ...
    const ensureIdForElement = React.useCallback((el) => {
    ...
    React.useEffect(() => {
        ...
        const refreshSystemTargets = async () => {
            try {
                const response = (await window.electron.listSystemSnapTargets());
    ...
    React.useEffect(() => {
        ...
        const loop = (ts) => {
            ...
            const ranked = targetsLive
                .map(target => ({
                target,
                score: targetScore(target, frame.pointerX, frame.pointerY, frame.velocityX, frame.velocityY, frame.suggestedId, frame.lockedId, lockedClusterId, frame.cooldownUntilByTarget, now),
            }))
