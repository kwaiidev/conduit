...
export function ServiceStartupStep({ service }: { service: ServiceKind }) {
  ...
  const [statusMessage, setStatusMessage] = React.useState(
    `Press Start to launch the ${SERVICE_LABEL[service]} backend terminal.`
  );
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h3 style={styles.title}>{SERVICE_LABEL[service]} Service</h3>
        <p style={styles.description}>
          Launch this backend from onboarding so the modality can be enabled later from Home/Overlay.
        </p>
        ...
        <button
          ...
          disabled={isStarting}
        >
          {isStarting ? "Starting..." : isReady ? "Running" : "Start"}
        </button>
      </div>
    </div>
  );
